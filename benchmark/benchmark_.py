...
def procedure(t, simu_params, kernel_params, kernel_name='raised_cosine',
    ...
    kernel_value, intensity_value, driver_tt, driver, acti_tt, acti = simu(
        kernel_params[0], kernel_params[1], kernel_params[2], kernel_params[3],
        simu_params=[simu_params[0], simu_params[1], simu_params[2]],
        isi=simu_params[3], kernel_name=kernel_name, seed=seed,
        plot_intensity=False, lower=kernel_params[4], upper=kernel_params[5])
    ...
    model = Model(t, baseline_init, alpha_init, m_init, sigma_init,
                  lower=kernel_params[4], upper=kernel_params[5])
    res_dict = training_loop(model,  driver, acti, simu_params[4],
                             simu_params[5], simu_params[6],
                             test=0.2, device=device)
    wanted_keys = ['est_params', 'n_test']
    hist_dic.update(dict((k, res_dict[k])
                         for k in wanted_keys if k in hist_dic))
